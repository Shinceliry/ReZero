...
class BandSequenceModelModule(nn.Module):
    ...
    def __init__(
            ...
            hidden_dim_size: int,
            rnn_type: str = 'lstm',
            bidirectional_T: bool = True,
            bidirectional_K: bool = True,
            num_layers: int = 12,
    ):
        ...
        for _ in range(num_layers):
            rnn_across_t = RNNModule(
                input_dim_size, hidden_dim_size, rnn_type, bidirectional_T
            )
            rnn_across_k = RNNModule(
                input_dim_size, hidden_dim_size, rnn_type, bidirectional_K
            )
            self.bsrnn.append(
