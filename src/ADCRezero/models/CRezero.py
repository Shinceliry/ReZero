...
class CRezeroModel(nn.Module):
    ...
    def __init__(self, args, device: str = "cuda", return_mask: bool = False, complex_as_channel: bool = True):
        super().__init__()
        if not args.region_type == 'conical':
            raise ValueError("CReZero model only supports 'conical' region type.")
    ...
    def forward(self, mix: torch.Tensor,
        ...
        # Feature extraction
        feats = self.fe(mix, theta_l, theta_h)
        spec_full = feats['spec']               # [B, C, F, T]
        spec_bands = feats['spec_bands']        # List of [B, C, (e-s), T]
        ipd_bands = feats['ipd_bands']          # List of [B, Mp, (e-s), T]
        ild_bands = feats['ild_bands']          # List of [B, Mp, (e-s), T]
        V_bands = feats['V_theta_bands']        # List of [B, Nviews, (e-s), T]
        ...
        for k, (s, e) in enumerate(self.subbands):
            ...
            ragg = self.region_aggs[k](V)                               # [B*T_spec, N_agg*P] (N_aggは model.py65行目 参照, P=region_dim)
            ...
            merged = feat_spec_band_ref + feat_ipd + feat_ild + feat_reg + feat_dist
